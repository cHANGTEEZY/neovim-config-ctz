...
do
  ...
  vim.keymap.set('n', '<leader>bo', '<cmd>%bd|e#|bd#<CR>', { desc = '[B]uffer close [O]thers' })

  -- Jump to listed buffer 1-9 (matches the tabline)
  for i = 1, 9 do
    vim.keymap.set('n', '<leader>' .. i, function()
      local bufs = vim.tbl_filter(function(b) return vim.api.nvim_buf_is_valid(b) and vim.bo[b].buflisted end, vim.api.nvim_list_bufs())
      if bufs[i] then vim.api.nvim_set_current_buf(bufs[i]) end
    end, { desc = 'Go to buffer ' .. i })
  end

  -- Ctrl/Cmd + click: move cursor, then go to definition (imports, symbols)
  local function mouse_goto_definition()
  ...
  vim.keymap.set('n', '<C-LeftMouse>', mouse_goto_definition, { desc = 'Ctrl-click go to definition' })
  vim.keymap.set('n', '<D-LeftMouse>', mouse_goto_definition, { desc = 'Cmd-click go to definition' })

  -- Mouse back / forward after jumping to a definition
...
do
  ...
  -- Comment / uncomment with `gc` (line) and `gcc` (current line)
  require('mini.comment').setup()

  -- Buffer tabs at the top of the editor
  require('mini.tabline').setup()

  -- Close buffers without wrecking the window layout
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      -- To jump back, press <C-o>. Cmd/Ctrl-click also uses this.
...
do
  ...
  local servers = {
    ...
    -- ESLint diagnostics + "fix this" code actions in TS/JS
    -- Formatting is left to prettier via conform.nvim
    eslint = {
      settings = {
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      -- Disable autoformat for files that are too large
